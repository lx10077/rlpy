...
from config.ftrpo.ftrpo_opt import FTrpoUpdater
...
parser.add_argument('--alpha', type=float, default=2, metavar='G',
                    help='alpha for f-divergence (default: 1)')
...
cfg = Cfg(parse=args)
agent = ActorCriticAgent("FTrpo" + args.dis, env_factory, policy_net, value_net, cfg,
                         running_state=running_state)
ftrpo = FTrpoUpdater(policy_net, value_net, cfg, use_fim=False)
evaluator = ActorCriticEvaluator(agent, cfg)
trainer = ActorCriticTrainer(agent, ftrpo, cfg, evaluator)
trainer.start()
